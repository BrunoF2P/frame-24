...
- [x] **`float64` → `int64` (centavos):** Migrado para `money.Cents`/`money.Subcent` (int64) em todos os contextos (catalog, operations, sales, inventory, finance, payments, fiscal e subscribers) com colunas `BIGINT` via migration `0007`; validações exatas sem epsilon; contrato JSON preserva decimal.
