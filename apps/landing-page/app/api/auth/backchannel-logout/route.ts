...
import { revokeOidcSessionFromBackchannel } from "@/lib/internal-oidc-session";
...
export async function POST(request: Request) {
  try {
    ...
    await revokeOidcSessionFromBackchannel({
      logoutToken,
      expectedAudience: process.env.AUTH_OIDC_ID ?? "frame24-landing",
    ...
    console.error("[backchannel-logout][landing]", message);
